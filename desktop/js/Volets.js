...
function saveEqLogic(_eqLogic) {
	...
	if (typeof( _eqLogic.cmd) !== 'undefined') {
		for(var index in  _eqLogic.cmd) { 
			 _eqLogic.cmd[index].configration.action=$('#tab_' +init(_eqLogic.cmd[index].id+' .ActionIn')).getValues('.expressionAttr');
			 _eqLogic.cmd[index].configration.action=$('#tab_' +init(_eqLogic.cmd[index].id+' .ActionOut')).getValues('.expressionAttr');
		}
	}
    return _eqLogic;
}
...
function AddZone(_zone){
	...
	var NewMode = $('<div style="margin-right:20px" class="cmd tab-pane tabAttr" id="tab_' +init(_zone.id) + '">')	
		.append($('<div class="row">')
			...
			.append($('<input class="cmdAttr" data-l1key="configuration" data-l2key="action" style="display : none;" />'))
